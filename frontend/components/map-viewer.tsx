...
import { MapPin, X, Loader2, Navigation, Eye, Clock, Ruler } from "lucide-react"
...
export function MapViewer({ videoId, videoName, onClose }: MapViewerProps) {
  ...
  useEffect(() => {
    ...
    sorted.forEach((p, i) => {
      ...
      const isSelected = selectedPoint?.frameNumber === p.frameNumber
      ...
      // Outer ring
      ctx.beginPath()
      ctx.arc(sp.x, sp.y, isHovered || isSelected ? 8 : 5, 0, Math.PI * 2)
      ctx.fillStyle = isHovered || isSelected ? color : `${color}cc`
      ctx.fill()

      // Inner dot
      ctx.beginPath()
      ctx.arc(sp.x, sp.y, isHovered || isSelected ? 4 : 2.5, 0, Math.PI * 2)
      ctx.fillStyle = "#0a0f1a"
      ctx.fill()
  ...
  }, [points, scale, offset, hoveredPoint, selectedPoint, worldToScreen, screenToWorld])
    ...
    (e: React.MouseEvent) => {
      ...
      if (found) {
        setSelectedPoint((prev) =>
          prev?.frameNumber === found!.frameNumber ? null : found
        )
      } else {
  ...
  return (
    <div className="relative flex flex-1 flex-col overflow-hidden rounded-xl border border-border bg-card">
      ...
      <div ref={containerRef} className="relative flex-1 overflow-hidden">
        ...
        {selectedPoint && (
          <div className="absolute top-3 right-3 z-20 w-72 rounded-xl border border-white/10 bg-black/90 backdrop-blur-xl shadow-2xl overflow-hidden">
            {/* Panel header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
              <div className="flex items-center gap-2">
                <Eye className="h-3.5 w-3.5 text-white/60" />
                <span className="text-xs font-semibold text-white">Frame {selectedPoint.frameNumber}</span>
              </div>
              <button
            ...
            {/* Metadata row */}
            <div className="flex items-center gap-3 px-4 py-2 border-b border-white/5 text-[10px] text-white/40">
              <span className="flex items-center gap-1">
                <Clock className="h-3 w-3" />
                {selectedPoint.seconds.toFixed(1)}s
              </span>
            ...
            <div className="max-h-64 overflow-y-auto">
              {selectedPoint.detections && selectedPoint.detections.length > 0 ? (
                <div className="divide-y divide-white/5">
                  {selectedPoint.detections.map((det, i) => (
                    <div key={i} className="flex items-start gap-3 px-4 py-2.5 hover:bg-white/5 transition-colors">
                      <span
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {det.distanceEstimate != null && (
                          <span className="flex items-center gap-1 mt-1 text-[10px] text-white/30">
                            <Ruler className="h-2.5 w-2.5" />
                            {det.distanceEstimate.toFixed(1)}m estimated distance
                          </span>
                        )}
                      </div>
                    </div>
                ...
                <div className="px-4 py-3">
                  <p className="text-xs text-white/60">{selectedPoint.description}</p>
                </div>
              )}
            </div>
            ...
            {selectedPoint.detections && selectedPoint.detections.length > 0 && (
              <div className="px-4 py-2 border-t border-white/5 text-[10px] text-white/30">
                {selectedPoint.detections.length} item{selectedPoint.detections.length !== 1 ? "s" : ""} detected
              </div>
            )}
          </div>
        )}
